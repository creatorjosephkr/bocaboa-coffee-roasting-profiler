<p align="center">
  <img src="/joseph.jpg" width="60%" alt="크리에이터 요셉" /><br />
...
보카보카 250BT/500BT 커피 로스팅기를 위한 실시간 크로스 플랫폼(macOS & Windows) 로스팅 프로파일러 애플리케이션입니다.  
...
### ChatGPT와의 협조를 통해 인공지능 로스팅 가이드 기능 지원
...
A real-time cross-platform (macOS & Windows) roasting profiler application designed for the **BocaBoca 250BT/500BT** coffee roasters.  
...
### v2.0 (2026.07.10)
* **맥/윈도우즈 멀티 플랫폼 동시 지원**: 일렉트론(Electron) 프레임워크로 전면 재구축하여 macOS와 Windows 환경을 동시에 완벽 지원합니다.
* **깃허브 저장소를 이용한 프로파일 공유**: 로스팅 데이터를 깃허브 공용(Public) 및 개인(Private) 저장소와 실시간 연동하여 손쉽게 공유 및 백업할 수 있습니다.
* **모의 로스팅 기능**: 저장된 세션 데이터를 기반으로 과거 로스팅 과정을 실시간 배속(최대 60배속)으로 재현하는 시뮬레이션 기능이 추가되었습니다. (Space 키로 일시정지/재개 지원)
* **30초 단위 타임테이블**: 메인 대시보드 및 상세 분석 리포트에 30초 단위의 주요 온도 테이블을 자동으로 생성하여 인쇄 및 기록 확인 편의성을 높였습니다.
* **BLE Scanner 내장**: 블루투스 기기를 검색하고 등록할 수 있는 전용 스캐너 UI가 추가되었습니다.
* **음성 안내 서비스**: 예열 상태 및 각 로스팅 이벤트(1차/2차 팝 등) 시 목소리로 실시간 상황을 안내합니다.
* **RoR 그래프 및 터닝포인트 정밀화**: 상승률(RoR) 연산 공식을 더 정교하게 다듬고, 터닝포인트(TP) 자동 감지 및 시각 보정 기능을 탑재했습니다.
* **인터페이스 최적화 및 줌인 조절**: 로스팅 진행 중 실시간 메인 그래프의 보기 비율을 확대/축소(휠 스크롤 및 상단 툴바 버튼)할 수 있으며, 확대 상태에서 드래그하여 패닝(좌우 스크롤)이 가능합니다.
* **단축키 및 수동 열량 조절 개선**: 키보드 단축키를 보완하고 수동 열량 조절 반응성을 극대화하여 오동작을 줄이고 사용성을 개선했습니다.
...
### v2.0 (2026.07.10) - English Update Notes
* **Cross-Platform Support**: Rebuilt with Electron to run natively on both macOS and Windows.
* **GitHub Profile Sharing**: Sync and backup profiles using GitHub Public and Private repositories.
* **Mock Roasting Simulator**: Simulate past roasting profiles in real-time (up to 60x speed) with pause/resume support using the Space key.
* **30-Second Timetable**: Automatically generates a structured 30s-interval temperature grid on both main dashboard and report modal.
* **Integrated BLE Scanner**: A dedicated BLE scanner UI to easily discover and bind your roaster.
* **Voice Assistant**: Audible notifications guide you through preheating status and roasting events (Charge, Cracks, etc.).
* **Sophisticated RoR & Auto TP Calibration**: Refined the Rate of Rise algorithm and integrated automatic Turning Point detection and scale shift.
* **Interactive Chart Zooming**: Zoom in/out on the live chart using mouse wheel or top toolbar buttons, and pan left/right by dragging.
* **Improved Hotkeys & UI Polish**: Refined keyboard shortcuts, optimized manual heat control responsiveness, and polished the overall interface.
