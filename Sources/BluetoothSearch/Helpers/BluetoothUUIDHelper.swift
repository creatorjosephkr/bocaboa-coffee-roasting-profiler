...
// MARK: - Standard BLE UUID Lookup Tables

struct BluetoothUUIDHelper {

    // MARK: - GATT Service Names
    ...
    // MARK: - GATT Characteristic Names
    ...
    static func serviceName(for uuid: CBUUID) -> String? {
        let key = shortKey(from: uuid)
        return serviceNames[key]
    }

    static func characteristicName(for uuid: CBUUID) -> String? {
        let key = shortKey(from: uuid)
        return characteristicNames[key]
    }

    static func serviceDisplayName(for uuid: CBUUID) -> String {
        serviceName(for: uuid) ?? uuid.uuidString
    }

    static func characteristicDisplayName(for uuid: CBUUID) -> String {
        characteristicName(for: uuid) ?? uuid.uuidString
    }

    private static func shortKey(from uuid: CBUUID) -> String {
        let str = uuid.uuidString.uppercased()
        // Standard BLE UUIDs look like "0000XXXX-0000-1000-8000-00805F9B34FB"
        if str.count == 36 {
