...
# Add targetAgtron if missing
if "@State private var targetAgtron" not in content:
    content = content.replace("@State private var showSessionBrowser = false", "@State private var showSessionBrowser = false\n    @State private var targetAgtron: Double = 65.0")
                        ...
                        VStack(spacing: 8) {
                            ...
                            HStack {
                                Text("투입 온도")
                                    ...
                                    .foregroundColor(.textSecondary)
                                    .frame(width: 80, alignment: .leading)
                                TextField("목표 투입온도 (선택)", text: $manager.preheatTemp)
                                    .textFieldStyle(.plain)
                                    .font(.system(size: 12))
                                    .padding(6)
                                    .background(Color.appBackground)
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            ...
                            Text("아그트론 색상 예측 (목표치)")
                                .font(.system(size: 12, weight: .bold))
                            Spacer()
                            Text("\\(Int(targetAgtron))")
                                .font(.system(size: 14, weight: .black, design: .monospaced))
                                .foregroundColor(.appPrimary)
                        }
                        
                        let detail = agtronDetail(for: targetAgtron)
                        HStack {
                            Text(detail.range)
                        ...
                        Slider(value: $targetAgtron, in: 25...95, step: 1)
                            .tint(.appPrimary)
                    }
                    .padding(12)
                    .background(Color.appSurface)
